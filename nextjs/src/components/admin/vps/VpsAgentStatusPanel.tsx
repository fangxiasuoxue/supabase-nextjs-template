...
function ProgressBar({ value, warn = 80, crit = 95, label, unit = '%' }: {
  ...
  const color = value >= crit ? 'bg-red-500' : value >= warn ? 'bg-orange-500' : 'bg-green-500'
  return (
...
export function VpsAgentStatusPanel({ vpsId, lastSyncAt, lastSyncBatchId }: Props) {
  ...
  return (
    <div className="space-y-6">
      {/* 实时指标 */}
      <div className="glass-card-premium p-6 rounded-2xl border border-slate-200 shadow-sm space-y-5">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="glass-card-premium p-4 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <span className="tech-mono text-lg font-black text-green-600">{formatBytes(status.rx_bytes)}</span>
        </div>
      </div>

      {/* 服务状态 */}
      {status.service_states && Object.keys(status.service_states).length > 0 && (
        <div className="glass-card-premium p-5 rounded-2xl border border-slate-200 shadow-sm space-y-3">
            ...
            {Object.entries(status.service_states).map(([svc, state]) => (
              <div key={svc} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className={`text-[10px] font-black uppercase ${state === 'active' ? 'text-green-600' : 'text-red-600'}`}>
                    {state}
                  </span>
                </div>
              </div>
      ...
      {/* 批量同步状态 */}
      {lastSyncAt && (
        <div className="pt-4 border-t border-slate-200 flex items-center justify-between text-[10px] text-muted-foreground/50">
          ...
          <span className="tech-mono flex items-center gap-2">
            ...
            {Date.now() - new Date(lastSyncAt).getTime() > 4 * 3600 * 1000 && (
              <span className="text-amber-600">⚠ 同步可能延迟</span>
            )}
          </span>
        </div>
      )}
